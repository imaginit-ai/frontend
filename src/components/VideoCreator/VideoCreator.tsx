...
const formSchema = z.object({
  prompt: z.string().trim().min(1, { message: "Please enter a prompt" }).max(50, {
    message: "Please enter a prompt with less than 50 characters. We recommend keeping it short and sweet.",
  }),
});
    ...
    <div className="w-3/4">
      <div className="glass rounded-xl flex flex-col w-full p-6 transition-all duration-300">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-full">
                  <FormControl>
                    <Input placeholder="Ask me to explain anything..." autoComplete="off" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <Button className="icon-button" type="submit" disabled={!form.formState.isValid || state === VideoCreatorState.GeneratingVideo}>
              Generate
        ...
        <Skeleton
          ...
          style={{
            maxHeight: state === VideoCreatorState.GeneratingVideo ? "1000px" : "0px",
            marginTop: state === VideoCreatorState.GeneratingVideo ? "20px" : "0px",
        ...
        >
          <div
            ...
            style={{
              display: state === VideoCreatorState.GeneratingVideo ? "flex" : "none",
            }}
          >
            <TypographyP className="text-center text-muted-foreground text-xl">Generating video...</TypographyP>
            <TypographyP className="text-center text-muted-foreground text-sm">This can take up to 2 minutes.</TypographyP>
            {/* <Progress
              value={progressValue}
      ...
      {state === VideoCreatorState.Idle && videoData?.url && (
        <div className="w-full flex justify-center items-center mt-5 overflow-hidden rounded-md glass">
          <video className="w-full rounded-md" src={videoData.url} controls={true} autoPlay={true} />
        </div>
      )}
    </div>
  );
